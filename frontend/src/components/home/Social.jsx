...
export function Testimonials() {
  return (
    <section className="py-24 md:py-36 bg-surface border-t border-white/10" data-testid="testimonials-section">
      <div className="max-w-[1320px] mx-auto px-6 md:px-10">
        <SectionHeader eyebrow="Customer voices" title="Trusted by the people who decide." />
        <div className="grid md:grid-cols-3 gap-5">
          {TESTIMONIALS.map((t, i) => (
            <Reveal key={t.name} i={i}>
              <div className="h-full border border-white/10 bg-bg/50 p-7 flex flex-col hover:border-signal/40 transition-colors">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 border border-white/20 flex items-center justify-center font-mono text-sm text-signal">{t.initials}</div>
                  <div>
...
export function Standards() {
  return (
    <section className="py-24 md:py-36 border-t border-white/10" data-testid="standards-section">
      <div className="max-w-[1320px] mx-auto px-6 md:px-10">
        <SectionHeader
        ...
        <div className="grid md:grid-cols-4 gap-x-8 gap-y-12">
          {STANDARDS.map((s, i) => (
            <Reveal key={s.group} i={i}>
              <h6 className="font-mono text-[11px] uppercase tracking-wider text-ink3 mb-5 pb-3 border-b border-white/10">{s.group}</h6>
