...
export default function RegulatoryReality() {
  return (
    <section id="regulatory" className="py-24 md:py-36 border-t border-ink/10" data-testid="regulatory-section">
      <div className="max-w-[1320px] mx-auto px-6 md:px-10">
        ...
        <RevealGroup className="grid grid-cols-1 md:grid-cols-12 gap-4">
          {REGIONS.map((r, i) => (
            <RevealItem
              key={r.code}
              i={i}
              className={`${i < 2 ? "md:col-span-6" : "md:col-span-4"} group border border-ink/10 hover:border-signal/50 p-7 bg-surface/40 transition-colors relative overflow-hidden`}
            >
              <div className="flex items-start justify-between mb-6">
                <span className="font-mono text-4xl font-bold text-ink/15 group-hover:text-signal/60 transition-colors">{r.code}</span>
                <span className={`font-mono text-[10px] uppercase tracking-wider px-2.5 py-1 border ${r.tone === "primary" ? "border-signal/40 text-signal" : "border-ink/15 text-ink3"}`}>
                  {r.status}
                </span>
              </div>
        ...
        {/* four forces */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-px mt-20 bg-ink/10 border border-ink/10">
          {FORCES.map((f, i) => (
