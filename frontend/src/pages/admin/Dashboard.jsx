...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-bg">
      <header className="border-b border-white/10 sticky top-0 bg-black/60 backdrop-blur-xl z-10">
        <div className="max-w-[1320px] mx-auto px-6 md:px-10 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button onClick={loadAll} className="text-ink2 hover:text-white flex items-center gap-1.5 text-sm"><RefreshCw className="w-4 h-4" /> Refresh</button>
            <button onClick={() => { logout(); navigate("/admin/login"); }} data-testid="admin-logout" className="text-ink2 hover:text-white flex items-center gap-1.5 text-sm"><LogOut className="w-4 h-4" /> Sign out</button>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-[1320px] mx-auto px-6 md:px-10 py-10">
        <div className="flex gap-1 border-b border-white/10 mb-10 overflow-x-auto">
          {TABS.map((t) => (
            <button key={t} onClick={() => setTab(t)} data-testid={`admin-tab-${t.toLowerCase()}`}
              className={`px-5 py-3 text-sm font-medium whitespace-nowrap transition-colors border-b-2 ${tab === t ? "border-signal text-white" : "border-transparent text-ink2 hover:text-white"}`}>
              {t}
            </button>
          ))}
        </div>
        ...
        {tab === "Content" && (
          <div>
            ...
            {showForm && (
              <form onSubmit={createResource} className="border border-white/10 bg-surface p-6 mb-8 grid md:grid-cols-2 gap-4" data-testid="admin-resource-form">
                <label className="text-sm md:col-span-1">
                  <span className="block font-mono text-[10px] uppercase tracking-wider text-ink3 mb-2">Type</span>
                  <select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })} className="w-full bg-white/5 border border-white/10 px-3 py-2.5 text-sm outline-none focus:border-signal">
                ...
                <label className="text-sm">
                  ...
                  <input value={form.date_label} onChange={(e) => setForm({ ...form, date_label: e.target.value })} placeholder="Jun 2026" className="w-full bg-white/5 border border-white/10 px-3 py-2.5 text-sm outline-none focus:border-signal" />
                </label>
                <label className="text-sm">
                ...
                <div className="md:col-span-2 flex gap-3">
                  ...
                  <button type="button" onClick={() => setShowForm(false)} className="border border-white/20 px-6 py-2.5 text-sm">Cancel</button>
                </div>
              </form>
            )}
